...
**Note**
The `install.sh` script does **not currently work on Fedora 43 KDE Plasma**. During Section 2h, the installation of the `hyprland-qtutils` package fails. This issue occurs because Fedora 43 uses different dependency versions than those provided by the Hyprland packages in the COPR repository [`solopasha/hyprland`](https://copr.fedorainfracloud.org/coprs/solopasha/hyprland/).
